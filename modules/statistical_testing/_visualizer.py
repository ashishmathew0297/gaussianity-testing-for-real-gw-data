...
from ._dataset_loader import get_TimeSeries, calculate_transforms

def display_statistic_pvalue_histogram(pvalues: pd.DataFrame, stat_test: Literal["Shapiro-Wilk", "Jarque-Bera", "Kolmogorov-Smirnov"]="Shapiro-Wilk") -> None:
    '''
    A function to plot a histogram of the Shapiro, Jarque-Bera or Kolmogorov-Smirnov p-values from the input.
    ...
    plt.xlabel(f'{stat_test} p-value')
...
def display_sample_plots(data: pd.DataFrame, whitening_tw: int=10, observation_tw: float=1, srate: int=4096, show_unwhitened=False, save_path: str = "") -> None:
    ...
    timeseries = TimeSeries.read(data['timeseries_filepath'])
    ...
    q_scan, time_elapsed = calculate_transforms(timeseries, "q_transform")
...
def display_section_statistics(data: pd.DataFrame, gpstimekey: str = "GPStime", stat_test: Literal["Shapiro", "JB", "KS", "Anderson", "CVM", "normal_test", "Lilliefors"]="Shapiro", section_size_seconds: float=1) -> None:
    '''
    ...
    - **stat_test:** The test being performed on the sections (values=["Shapiro", "JB", "KS", "Anderson", "CVM", "normal_test", "Lilliefors"]). Default="Shapiro".
...
def display_confusion_matrix(data: pd.DataFrame, stat_test: Literal["Shapiro", "JB", "KS", "Anderson", "CVM", "normal_test", "Lilliefors"]="Shapiro",per_glitch:bool=False, save_img: bool=False, output_folder: str="./outputs", filename: str="") -> None:
    '''
    ...
    disp = metrics.ConfusionMatrixDisplay(generate_confusion_matrix(data,stat_test).astype(int), display_labels=["Glitch Present", "Glitch Not Present"],)
    disp.plot()
    plt.grid(False)
    plt.title(title)
    plt.tight_layout()
...
def display_auc_roc(
        data: pd.DataFrame,
        stat_test: Literal["Shapiro", "JB", "KS", "Anderson", "CVM", "normal_test", "Lilliefors"]="Shapiro",
        thresholds: list[float] = [], save_img: bool=False, output_folder: str="./outputs") -> None:
    '''
    ...
    Inputs:
    - `data`: A dataframe containing either p-values for Shapiro-Wilks/Kolmogorov-Smirnov test or Statistic and Critical values for Anderson-Darling test.
    ...
    if stat_test == "Shapiro":
        ...
        title = "Jarque-Bera Receiver Operating Characteristic"
    elif stat_test == "KS":
        title = "Kolmogorov-Smirnov Receiver Operating Characteristic"
    elif stat_test == "ED":
        title = "Energy Distance Receiver Operating Characteristic"
    elif stat_test == "normal_test":
        title = "Normal Test Receiver Operating Characteristic"
    elif stat_test == "CVM":
        title = "Cramér-von Mises Receiver Operating Characteristic"
    elif stat_test == "Anderson":
        title = "Anderson-Darling Receiver Operating Characteristic"
    elif stat_test == "Lilliefors":
        title = "Lilliefors Receiver Operating Characteristic"
    ...
    if not len(thresholds):
        if stat_test != "Anderson":
            y_pred = data[f'{stat_test.lower()}_pvalue'].apply(lambda x: 1 if x <= 0.05 else 0)
        else:
            y_pred = data.apply(lambda x: 1 if x["ad_statistic"] > x["ad_critical_values"][2] else 0, axis=1)
        fpr, tpr, _ = metrics.roc_curve(data['glitch_present'], y_pred)
        roc_auc = metrics.auc(fpr, tpr)
        filename = f"auc_roc_{stat_test}_p005.pdf"
    # Using input thresholds
    else:
        fpr.append(0)
        tpr.append(0)
        if stat_test in ["Shapiro", "JB", "KS", "CVM", "normal_test", "Lilliefors"]:
            for i in range(len(thresholds)):
                y_pred = data[f'{stat_test.lower()}_pvalue'].apply(lambda x: 1 if x <= thresholds[i] else 0)
                fpr_temp, tpr_temp, _ = metrics.roc_curve(data['glitch_present'], y_pred)
            ...
            significance_levels = np.array(data['ad_significance_level'][0]) / 100.0
